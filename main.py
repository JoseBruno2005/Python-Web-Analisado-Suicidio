...
def salvarcorrelacao():
    ...
    if 'submitSP' in request.form:
        ...
        if dao.insert_correlacao(dao.conectardbrender(), 'pib', correlacao):
        ...
        if dao.insert_correlacao(dao.conectardbrender(), 'idh', correlacao):
        ...
        if dao.insert_correlacao(dao.conectardbrender(), 'ideb', correlacao):
...
def login():
    ...
    if dao.login(email, senha, dao.conectardbrender()):
...
def redirecionarcadastro():
    if request.method == 'GET':
        ...
        email = str(request.form.get('email'))
        senha = str(request.form.get('senha'))
        if dao.inserirusuer(email, senha, dao.conectardbrender()):
            return redirect(url_for('exibir_grafico_continente'))
        else:
